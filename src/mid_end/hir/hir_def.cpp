...
{
    return std::visit(
        Overload{[](HIRAssign& assign) -> std::string {
                 ...
                 [](HIRUnaryOp& unary) -> std::string {
                     return "T" + std::to_string(unary.reg.value) + " = " +
                            operator_print(unary.op) + " " + hir_expr_print(unary.fac);
                 },
                 [](HIRBinaryOp& binary) -> std::string {
                     return "T" + std::to_string(binary.reg.value) + " = " +
                            hir_expr_print(binary.lhs) + " " + operator_print(binary.op) + " " +
                 ...
                 [](HIRCondJump& condjump) -> std::string {
                     return "IF_FALSE " + hir_expr_print(condjump.condition) + " GOTO L" +
                            std::to_string(condjump.label.value);
                 },
                 ...
                 [](HIRLoad& load) -> std::string {
                     return "T" + std::to_string(load.reg.value) + " = LD " +
                            std::string(load.source) + "_" + std::to_string(load.scope_id);
                 },
                 [](HIRStore& store) -> std::string {
                     if(std::holds_alternative<int>(store.reg))
                     {
                         int val = std::get<int>(store.reg);
                         return "ST " + std::string(store.dest) + "_" +
                                std::to_string(store.scope_id) + ", " + std::to_string(val);
                     }
                     else if(std::holds_alternative<VirtualRegisterID>(store.reg))
                     {
                         VirtualRegisterID vreg = std::get<VirtualRegisterID>(store.reg);
                         return "ST " + std::string(store.dest) + ", " + "T" +
                                std::to_string(vreg.value);
                     }
